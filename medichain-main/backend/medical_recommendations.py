...
def personalize_treatment(base_recommendations, patient_data):
    ...
    # Age-based modifications with safe conversion
    try:
    ...
    # Handle different allergy input formats
    if isinstance(allergies, str):
        # If allergies is a string like "None" or "Penicillin, Aspirin"
        if allergies.lower() in ['none', 'none reported', 'no allergies', '']:
            allergies = []
        else:
    ...
    if allergies and len(allergies) > 0:
        ...
        personalized['allergy_warnings'] = "No known allergies reported. Standard medication precautions apply."
