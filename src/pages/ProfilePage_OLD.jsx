...
const ProfilePage = () => {
  ...
  // Immediate redirect if doctor tries to access patient profile
  useEffect(() => {
    if (user?.profile?.role === 'doctor' || user?.role === 'doctor') {
  ...
  const loadProfile = useCallback(async () => {
    ...
    try {
      ...
      // Check if user is a patient - if doctor, redirect to doctor profile
      if (user.profile?.role === 'doctor') {
        console.log('🚨 Doctor detected on patient profile page - redirecting to doctor profile');
