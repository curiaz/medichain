...
TREATMENT_RECOMMENDATIONS = {
    ...
    "Asthma": {
        "medications": [
            ...
            {
                "name": "Prednisone",
                "dosage": "40-60mg daily",
                "duration": "5-7 days",
                "type": "oral steroid (severe cases)",
            },
        ],
        "treatments": [
...
def get_personalized_recommendations(diagnosis, patient_data=None):
    ...
    """
    base_recommendations = TREATMENT_RECOMMENDATIONS.get(
        diagnosis,
        {
            "medications": [],
            "treatments": [
...
def personalize_treatment(base_recommendations, patient_data):
    ...
    if allergies:
        personalized["allergy_warnings"] = (
            f"Patient allergies noted: {', '.join(allergies)}. Avoid contraindicated medications."
        )
    ...
    if current_medications:
        personalized["interaction_warning"] = (
            f"Review for drug interactions with current medications: {', '.join(current_medications)}"
        )
