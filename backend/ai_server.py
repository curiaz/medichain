...
def enhanced_confident_diagnosis():
    ...
    try:
        ...
        enhanced_parsed_result = {
            ...
            "confidence_keywords_found": _count_confidence_keywords(
        ...
        boosted_confidence, applied_boosts = confidence_booster.boost_confidence(
            base_confidence,
            enhanced_parsed_result,
            list(result["feature_vector"].values()),
        )
        ...
        try:
            recommendations = get_personalized_recommendations(
                result["primary_diagnosis"], {"symptoms": result["parsed_symptoms"]}
            )
        ...
        result["analysis_summary"] = {
            ...
            or result.get("manual_intensity_override", False),
            "confidence_improvement": (
                f"+{((boosted_confidence - base_confidence) * 100):.1f}%"
                if boosted_confidence > base_confidence
                else "No boost applied"
            ),
        }
